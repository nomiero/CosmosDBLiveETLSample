            ...
            while (true) {
                ...
                if (currentMigrationId == null) { 
                    var configDocs = client.CreateDocumentQuery<MigrationConfig>(UriFactory.CreateDocumentCollectionUri(jobdb, jobColl),
                        string.Format("select * from c where NOT c.completed")).AsEnumerable<MigrationConfig>().ToList();
                ...
                {
                    var configDocs = client.CreateDocumentQuery<MigrationConfig>(UriFactory.CreateDocumentCollectionUri(jobdb, jobColl),
                        string.Format("select * from c where c.id = \"{0}\"", currentMigrationId)).AsEnumerable<MigrationConfig>().ToList();
